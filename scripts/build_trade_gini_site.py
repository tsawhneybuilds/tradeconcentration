...
def build_cadot_integrated_body(data: dict[str, Any]) -> str:
    ...
    <section class="section" id="cadot-latest-mechanisms">
      ...
      <div class="note">
        <p><strong>How to interpret the exit-cone image:</strong> Think of a product's PRODY as the income level of the countries that typically export it. At zero on the horizontal axis, the country's income matches the product's typical exporter. Moving to the right means the country is richer than the countries normally associated with that product—the product increasingly looks like something from an earlier stage of development.</p>
        <p><strong>What the orange line shows:</strong> Orange represents the richest quarter of observed country-years, those above approximately $39,308 per person in constant-2021 PPP dollars. As these observations move farther right, product exit rises. In plain English, richer countries are more likely to stop exporting products that are increasingly out of step with their income level. A one-unit move right—equivalent to multiplying the income-to-PRODY ratio by about 2.7—is associated with a 3.25-percentage-point larger rich-side exit effect (raw p&lt;0.001).</p>
        <p><strong>Important caution:</strong> The orange line is generally below the gray line, so the graph does not say that rich countries abandon more products overall. It says that income–product mismatch matters more for which products rich countries abandon. The regression accounts for persistent country and product differences and common base-year shocks, but it remains descriptive rather than causal. “Rich side” means the observed top income quarter because the model's estimated turning point lies beyond the income range well represented in the data.</p>
      </div>
      <div class="download-grid compact-downloads">
